...
def add_employee(filename, cur, conn):
    ...
    for item in data:
        ...
        cur.execute('INSERT OR IGNORE INTO employees (employee_id, first_name, last_name, job_id, hire_date, salalry) values (?,?,?,?,?,?)', (emp_id, f_name, l_name, hire_date, job_id, salary))
        conn.commit()
...
def problematic_salary(cur, conn):
    cur.execute('SELECT employee_id FROM employees JOIN jobs ON job_id')
